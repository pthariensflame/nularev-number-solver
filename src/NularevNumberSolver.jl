...
struct Term
    ...
    fives::UInt
    sevens::UInt
    Term(; twos = 0, threes = 0, fives = 0, sevens = 0) = new(
        convert(UInt, twos),
        convert(UInt, threes),
        convert(UInt, fives),
        convert(UInt, sevens),
...
Base.convert(::Type{T}, term::Term) where {T<:Number} = *(
    convert(T, 2)^(term.twos),
    convert(T, 3)^(term.threes),
    convert(T, 5)^(term.fives),
    convert(T, 7)^(term.sevens),
)
(::Type{T})(term::Term) where {T<:Number} = convert(T, term)
Base.:*(x::Term, y::Term) = Term(
    twos = x.twos + y.twos,
    threes = x.threes + y.threes,
    fives = x.fives + y.fives,
    sevens = x.sevens + y.sevens,
)
...
Base.isone(terms::SumOfTerms)::Bool = isone(length(terms.terms)) && isone(only(terms.terms))
...
function translate(term::Term; isinitial::Bool)::Vector{String}
    ...
    return [
        fill("mem", term.twos)
        fill("let", term.threes)
        fill("zhain", term.fives)
        fill("wat", term.sevens)
    ]
end
